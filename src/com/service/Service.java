...
public class Service {
	...
	public String changeOrder(String out_trade_no,
			String transaction_id, String time_end) {
		String sql = "update orders (result,device_info,transaction_id,time_end)values(success,'"
				+ transaction_id
				+ "','"
				+ time_end
		...
		int ret = db.executeUpdate(sql);
		if (ret != 0) {
			String getPayInfo = "select * from orders where out_trade_no='"
					+ out_trade_no + "'";
	...
	public String confrimPayResult(String card_code) {
		String sql = "select * from orders where card_code='" + card_code + "'";
		DBManager db = DBManager.createInstance();
		db.connectDB();
